...
class EntitySpec {
  ...
  basePrices(queryParams, responseHandler) {
      ...
      if (response.contentType.startsWith("application/json")) {
        ...
        if (response.statusCode == 200) {
          if (msg["prices"] !== undefined) {
            response.body.prices = msg["prices"].map(
              (x) => new pricing_common.Price(x)
            );
          }
    ...
    this.context.request("GET", path, body, undefined, handleResponse);
  }
  ...
  get(accountID, queryParams, responseHandler) {
    ...
    if (typeof queryParams["includeUnitsAvailable"] !== "undefined") {
      path =
        ...
        queryParams["includeUnitsAvailable"] +
        "&";
    }
  ...
  candles(instrument, queryParams, responseHandler) {
    ...
    if (typeof queryParams["alignmentTimezone"] !== "undefined") {
      path =
        path + "alignmentTimezone=" + queryParams["alignmentTimezone"] + "&";
    }
      ...
      if (response.contentType.startsWith("application/json")) {
        ...
        if (response.statusCode == 200) {
          ...
          if (msg["candles"] !== undefined) {
            response.body.candles = msg["candles"].map(
              (x) => new instrument.Candlestick(x)
            );
          }
        } else if (response.statusCode == 400) {
        } else if (response.statusCode == 401) {
